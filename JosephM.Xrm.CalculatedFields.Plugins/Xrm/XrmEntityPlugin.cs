...
namespace JosephM.Xrm.CalculatedFields.Plugins.Xrm
{
    ...
    public abstract class XrmEntityPlugin : XrmPlugin
    {
        ...
        ///     NOT IMPLEMENTED FOR SET STATE
        /// </summary>
        public bool MeetsFilterChanging(FilterExpression filter)
        {
            var metPrePlugin = false;
            var metPostPlugin = false;
            switch (MessageName)
            {
                case PluginMessage.Create:
                    {
                        metPostPlugin = XrmEntity.MeetsFilter(GetField, filter);
                        break;
                    }
                case PluginMessage.Update:
                    {
                        metPrePlugin = XrmEntity.MeetsFilter(PreImageEntity.GetField, filter);
                        metPostPlugin = XrmEntity.MeetsFilter(GetField, filter);
                        break;
                    }
                case PluginMessage.Delete:
                ...
                default:
                    {
                        //not sure what to do for setstate with status
                        throw new InvalidPluginExecutionException(
                            "MeetsFilterChanging Not Implemented for plugin message " + MessageName);
                    }
            }
            return metPostPlugin != metPrePlugin;
        }
